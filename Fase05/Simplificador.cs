...
namespace Fase05
{
    class Simplificador
    {
        ...
        // Dictionary de todos los serializadores que ya tenemos
        private static Dictionary<Type, ISerializer> ListaSerializadores;

        // Variable principal: el tipo del que se va a crear el serializador
        private Type tipo;

        public Simplificador(Type tipo)
        {
            this.tipo = tipo;
            // Tipo de codificación por defecto
            Simplificador.tipoDeCodificacion = tiposDeCodificacion.XML;
        ...
        public dynamic getSerializer()
        {
            ...
            serializador = this.compile(this.tipo);
            return serializador;
        }
        ...
        private void generateSerializer()
        {
            ...
            this.getCabecera(tipo);

            // 2º Generar los métodos encode y decode (se hace a la vez)
            this.generateEncodeAndDecodeMethods(tipo);
            strCodigo += strEncode;
...
//            this.generateAuxiliarMethods();
        ...
        private void getCabecera(Type tipo)
        {
...
";
            ...
            strCodigo += tipo.Name + "Codec {";
            strCodigo += @"
        public static Type tipo = Type.GetType(""" + tipo.Name;
            strCodigo += @""");
        public static String nombreClase = """ + tipo.Name;
            strCodigo += @""";
        private static string TAB = ""\t"";
        private static string SALTO = ""\r\n"";";
        }
        ...
        private void generateEncodeAndDecodeMethods(Type tipo)
        {
            strEncode += @"
        public static void encode(" + tipo.FullName + " obj, ref String str){";
//            " + tipo.FullName + " obj = (" + tipo.Name;
//            strEncode += @")objeto;";
/*
            strEncode += @"
        public void encode(Clase01Basica obj, ref string str){";
*/ 
            strDecode += @"
        public static void decode(String str, ref " + tipo.FullName + " obj){";
            ...
            texto += """ + abrir("valor");
                            ...
                            else if (!t.FullName.StartsWith("System.")) // OBJETOS EXTENOS
                            {
                                ...
                                ISerializer elSerializador;
                                if(ListaSerializadores.TryGetValue(t, out elSerializador))
                                {
//                                    ISerializer.encode();
                                }
                            }
